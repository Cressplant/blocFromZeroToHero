...
setUp((){
   print('SETUP');
  counterCubit = CounterCubit(); // Initialise bloc/cubit
});
...
expect: [CounterState(counterValue: 1, wasIncremented: true)] // list of expected results, ordered
);


//! each test is called individually - setup, run, tearDown, repeat. Therefore each test should expect same initial value.

blocTest('The cubit should emit a CounterState(counterValue: 0, wasIncremented: false) when cubit.decrement() function is called.', 
build: () => counterCubit, // returns current instance being tested
act: (cubit) => cubit.decrement(), // action being applied to the cubit/bloc
expect: [CounterState(counterValue: -1, wasIncremented: false)] // list of expected results, ordered
);
